...
export default function LiquidEther({
  ...
  // Feature detection: Test WebGL support synchronously before render
  const webglSupported = useRef<boolean | null>(null);
  if (webglSupported.current === null) {
    try {
      const canvas = document.createElement('canvas');
      const gl = canvas.getContext('webgl') || canvas.getContext('experimental-webgl');
      webglSupported.current = !!gl;
      if (!gl) {
        console.warn('WebGL not supported, falling back to CSS background');
      }
    } catch (e) {
      console.warn('WebGL detection failed, falling back to CSS background:', e);
  ...
  useEffect(() => {
    if (!mountRef.current || !webglSupported.current) return;
